...
if ($action == "delete") {
    ...
    // 先查询出要删除的数据
    $sql = "SELECT * FROM {$pre_}department WHERE id IN ($ids)";
    $deletelist = all($sql);
    ...
    // 再查询出要删除部门下的所有员工
    $sql = "SELECT * FROM {$pre_}person WHERE depid IN ($ids)";
    $personlist = all($sql);
    ...
    // 删除部门
    $where = "id IN ($ids)";
    $affect = delete('department', $where);
...
// sql查询数据总数
$sql = "SELECT COUNT(id) AS c FROM {$pre_}department";
$count = find($sql);
// var_dump($count);
...
//查询部门数据
$sql = "SELECT * FROM {$pre_}department ORDER BY id DESC LIMIT $offset,$limit";
        ...
        <div class="container-fluid">
            <div class="row-fluid">
                <div class="btn-toolbar">
                    <button class="btn btn-primary" onClick="location='DepartmentAdd.php'">
                        ...
                        <tbody>
                            <?php foreach ($list as $item) { ?>
                                <tr>
                                    ...
                                    <td>
                                        <a href="DepartmentEdit.php?id=<?php echo $item['id']; ?>">
