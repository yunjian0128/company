...
// 获取部门名称
$depid = $job['depid'];
$sql = "SELECT * FROM {$pre_}department WHERE id = $depid";
$department = find($sql);
if (!$department) {
    Notice('当前编辑的部门不存在');
    exit;
...
if ($_POST) {
    ...
    $sql = "SELECT * FROM {$pre_}job WHERE id = $id AND name = '$name' AND depid = '$depid'";
    $info = find($sql);

    // 如果$info不为空，就说明未修改
    if ($info) {
        Notice("职位名称未修改", "JobList.php");
    }
                    ...
                    <div id="myTabContent" class="tab-content">
                        <div class="tab-pane active in" id="home">
                            <form method="post" enctype="multipart/form-data" class="form">
                                ...
                                <label>部门名称</label>
                                <input type="text" name="depname" placeholder="请输入部门名称" required class="input-xxlarge"
                                    value="<?php echo $depname ?>" />
